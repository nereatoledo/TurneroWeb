...
public interface PacienteRepository extends CrudRepository<Paciente, Integer> {
  ...
  @Query(
      "SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM Paciente p WHERE p.username ="
          + " :username")
  boolean existsByUsername(@Param("username") String username);

  @Query(
      "SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM Paciente p WHERE p.dni = :dni")
