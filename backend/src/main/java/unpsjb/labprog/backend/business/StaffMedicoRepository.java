...
public interface StaffMedicoRepository
  ...
  @Query(
      "SELECT CASE WHEN COUNT(s) > 0 THEN true ELSE false END FROM StaffMedico s WHERE"
          + " s.centro.nombre = ?1 AND s.medico.id = ?2")
  ...
  @Query(
      "SELECT s FROM StaffMedico s JOIN FETCH s.centro JOIN FETCH s.medico m LEFT JOIN FETCH"
          + " m.especialidad")
  ...
  @Query("SELECT s FROM StaffMedico s WHERE s.centro.id = ?1 AND s.medico.especialidad.id = ?2")
  List<StaffMedico> findByCentroAndEspecialidad(int centroId, int especialidadId);
}
