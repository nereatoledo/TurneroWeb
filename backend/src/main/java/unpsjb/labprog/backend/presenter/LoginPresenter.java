...
public class LoginPresenter {
  ...
  @Autowired private ObraSocialRepository obraSocialRepository;
  ...
  public ResponseEntity<?> registrarPaciente(@RequestBody Map<String, Object> datos) {
    ...
    if (username == null
      ...
      return ResponseEntity.badRequest()
          .body(Map.of("error", "Todos los campos obligatorios deben completarse"));
    }
    ...
    try {
      if (fechaNacStr != null && !fechaNacStr.isEmpty()) {
        java.time.LocalDate fecha = java.time.LocalDate.parse(
            fechaNacStr, DateTimeFormatter.ofPattern("dd/MM/yyyy"));
  ...
  public ResponseEntity<?> updatePaciente(
      @PathVariable Integer id, @RequestBody Map<String, Object> datos) {
    ...
    if (username != null && !username.trim().isEmpty()) {
      username = username.trim();
      if (!username.equals(paciente.getUsername())
          && pacienteRepository.existsByUsername(username)) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
    ...
    return ResponseEntity.ok(
        Map.of("mensaje", "Paciente actualizado con éxito", "username", paciente.getUsername()));
  }
}
